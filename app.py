...
def get_all_pages(method, params):
    all_items = []
    start = 0
    while True:
        p = dict(params)
...
def fetch_calls_split(day_start, day_end):
    ...
    Пробует специализированный метод телефонии; если недоступен — считает
    все звонки вместе (без разделения) через обычные CRM-активности."""
    global VOXIMPLANT_AVAILABLE

    if VOXIMPLANT_AVAILABLE is not False:
        ...
        if not error and items is not None:
            VOXIMPLANT_AVAILABLE = True
            incoming = {}
            outgoing = {}
            for item in items:
            ...
            if VOXIMPLANT_AVAILABLE is None:
                print(f"[!] voximplant.statistic.get недоступен ({error}). Дальше буду сразу использовать запасной вариант (без разделения вход/исход) — добавьте право 'Телефония (telephony)' в вебхук, чтобы включить разделение.")
            VOXIMPLANT_AVAILABLE = False
